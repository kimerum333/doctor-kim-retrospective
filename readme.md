...
이 프로젝트는 제가 선택한 주제가 아니라, 인턴 기간 중 "법률이나 의료 중
하나를 선택해 AI 챗봇을 만들어보라"는 요구로 시작됐습니다. 저는 두 도메인
모두 비전문가였고, 상대적으로 공공 데이터 접근성이 나아 보이는 의료 쪽을
선택했습니다.

지금 돌아보면, 이 출발점 자체가 프로젝트 전체의 한계를 결정지었습니다.
"무엇을 만들지"가 내부적 필요나 명확한 문제의식에서 나오지 않고 외부에서
주어진 채 시작된 프로젝트는, 아래 회고에서 다루는 모든 기술적 선택에서도
같은 문제를 반복합니다. 데이터의 품질을 판단할 수 없는 도메인, 사용자의
실제 질문 패턴을 모르는 상태에서의 RAG 설계, 그리고 "써보고 싶어서"
도입한 메시지 큐까지.

이 프로젝트의 가장 큰 교훈은 기술적인 것이 아니라 **"문제 정의가 비어 있는
프로젝트는 기술 선택의 나침반을 잃는다"**는 것이었습니다. 이후 프로젝트들은
"내가 이해하는 도메인에서, 내가 필요하다고 느끼는 기능을" 시작점으로 삼는
것을 원칙으로 하고자 합니다.
